...
from .models import StudentExtra, MidtermResult
...
@login_required(login_url='studentlogin')
@user_passes_test(is_student)
def midterm_result(request):
    student = get_object_or_404(StudentExtra, user_id=request.user.id, status=True)
    ...
    if midterm_results.exists():
        return render(request, 'school/midterm_result.html', {'mylist': midterm_results})
    else:
        return render(request, 'school/error.html', {'message': 'No midterm results found.'})
